...
export const generatePdfBuffer = async (html, pageSetup = {}) => {
  ...
  console.log(debugPrefix, '*** NEW PIPELINE ACTIVE ***');

  // Safer flags for Docker / Linux
  const launchArgs = [
    '--no-sandbox',
  ...
  console.log(debugPrefix, 'launchOptions =', {
    args: launchArgs,
    headless: true,
  });
...
@media print {
  ...
  [data-preview], [data-rm-preview] {
    display: none !important;
      ...
      // 3) Viewport + content loading (no networkidle0)
      ...
      // 4) Ensure screen styles and assets (fonts / images)
      ...
      // 5) Extra CSS for editable-field pseudo-elements
      try {
      ...
      // 6) Header rule only when needed
      try {
