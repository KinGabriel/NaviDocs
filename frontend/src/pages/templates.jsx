...
export default function TemplatesPage() {
  ...
  // Permissions (so this page works for ALL roles using /templates)
  ...
  const canApproveInline = ['secretary', 'dean'].includes(role);
  const canPublishInline = [
    'document controller',
  ...
  const initialStatus =
    ...
    (statusOptions.includes(searchParams.get('status')) &&
  ...
  const handleModalSubmit = async (templateFormData) => {
    if (!user || !canCreateTemplate) return;

    setLoading(true);
    try {
      const sizeKey = String(templateFormData.document_size || 'A4').toLowerCase();
      const paperSize =
        sizeKey === 'letter' ? 'Letter' : sizeKey === 'legal' ? 'Legal' : 'A4';
      ...
      // global editor route (still using document-controller editor)
      navigate(
        `/document-controller/create-template?templateId=${result.template._id}`,
      );
    } catch (error) {
  ...
  const fetchTemplates = async () => {
    ...
    try {
      ...
      if (selectedStatus !== 'All') {
        const allowed = (statusMap[selectedStatus] || []).map((s) =>
          s.toLowerCase(),
        );
      ...
      const lastActivity = (t) =>
        new Date(
          t.updatedAt ||
            t.updated_at ||
            t.status_meta?.last_activity_at ||
            t.last_activity_at ||
            t.createdAt ||
            t.created_at ||
            0,
  ...
  // Inline approve handler (only for secretary & dean)
  const handleInlineApprove = async (template) => {
    if (!user || !canApproveInline) return;
        ...
        prev.map((t) =>
          t._id === template._id
            ? {
                ...t,
                ...res.template,
                approvalMeta: res.approvalMeta || res.template?.approvalMeta,
              }
            : t,
        ),
      );
  ...
  // Inline publish handler (only for DC-related roles)
  const handleInlinePublish = async (template) => {
    if (!canPublishInline) return;
  ...
  return (
    <div className="min-h-screen bg-gray-200 flex flex-col">
      ...
      <div className="flex flex-1">
        {/* Sidebar is already global via MENU_CONFIG */}
        <Sidebar user={user} />
        ...
        <div className="flex-1 flex flex-col bg-white shadow pt-1 pb-4 px-4 md:px-8 mx-3 md:mx-6 mt-4 md:mt-8 rounded-xl overflow-x-hidden">
          <div className="flex-1 px-1 py-5">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-4">
              {/* Create button (only for allowed roles) */}
            ...
            <div className="flex flex-wrap gap-1 mb-4">
              {statusOptions.map((status) => {
                ...
                const style = isSelected
                  ? `${baseStyle} bg-blue-600 text-white border-blue-600 shadow-sm`
                  : `${baseStyle} bg-white text-gray-600 border-gray-300 hover:bg-gray-50 hover:border-gray-400`;
                ...
                templates.map((template, i) => (
                  <div key={template._id || i} className="min-w-0">
                    <TemplateCard
                      template={template}
                      user={user}
                      // Only pass approve / publish handlers if role allows it
                      onApprove={canApproveInline ? handleInlineApprove : undefined}
                      onPublish={canPublishInline ? handleInlinePublish : undefined}
                      onSelect={() =>
                        navigate(
                          `/document-controller/create-template?templateId=${template._id}`,
                        )
                      }
      ...
      {/* Create Template Modal (only useful if they can create) */}
      {canCreateTemplate && (
        <CreateTemplateModal
