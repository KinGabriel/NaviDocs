...
import {
  ...
  getTemplateVersionAPI,
  restoreTemplateVersionAPI,
  updateTemplateVersionBookmarkAPI,
  duplicateTemplateAPI,
  duplicateTemplateFromVersionAPI,
...
export default function TemplateVersionHistory({
    ...
    try {
      if (id) {
        ...
        if (rawVersions && rawVersions.length > 0) {
          const normalized = rawVersions.map((v, index) => {
            ...
            if (v?.last_activity_at) {
              if (
                ...
                lastActivity = v.last_activity_at;
              else if (v.last_activity_at.$date)
                lastActivity = v.last_activity_at.$date;
            ...
            if (v?.author) {
              ...
              if (typeof cb === 'object' && cb !== null) {
                ...
                else if (cb.first_name || cb.last_name)
                  author = [cb.first_name, cb.last_name]
                    .filter(Boolean)
                    .join(' ')
                    .trim();
                else if (cb.firstname || cb.lastname)
                  author = [cb.firstname, cb.lastname]
                    .filter(Boolean)
                    .join(' ')
                    .trim();
            ...
            const snapshotHeaderConfig =
              v?.snapshot?.headerConfig || v?.snapshot?.logoConfig || null;
            const snapshotPageSetup =
              v?.snapshot?.pageSetup || v?.snapshot?.page_setup || null;
            ...
            const stampDocCode =
              stampSrc.docCode ||
              stampSrc.document_code ||
              snap.document_code ||
              v.document_code ||
              '';
            const stampRevisionNo =
              stampSrc.revisionNo ||
              stampSrc.revision_no ||
              snap.revision_no ||
              v.revision_no ||
              '';
            const stampEffectivity =
              stampSrc.effectivity ||
              stampSrc.effectivity_date ||
              snap.effectivity ||
              snap.effectivity_date ||
              v.effectivity ||
              '';
          ...
          const latestFromGroup =
            grouped?.[0]?.items?.[0] || normalized[0];
  ...
  const groupVersionsByDate = (versionList) => {
    ...
    versionList.forEach((version) => {
      ...
      grouped[dateKey].push({
        ...
        versionName:
          version.versionName || version.name || version.note || '',
  ...
  const handleVersionSelect = async (version) => {
    ...
    if (version.content) {
      ...
      try {
        const data = await getTemplateVersionAPI(
          id,
          version.id
        );
        ...
        const fetchedFirst =
          Array.isArray(
            data?.version?.snapshot?.pages_json
          ) && data.version.snapshot.pages_json.length > 0
            ? data.version.snapshot.pages_json[0]
            : null;
        const snap = data?.version?.snapshot || {};
        const fetchedHeaderCfg =
          snap?.headerConfig || snap?.logoConfig || null;
        const fetchedPageSetup =
          snap?.pageSetup || snap?.page_setup || null;
        const stampSrc = snap?.documentStamp || {};
        const stampDocCode =
          stampSrc.docCode ||
          stampSrc.document_code ||
        ...
        const stampEffectivity =
          ...
          '';
        setVersionContent(
          fetchedFirst || data.version?.snapshot || currentContent
        );
        ...
        console.error(
          'Failed to fetch version content:',
          error
        );
  ...
  const filteredVersions = versions
    .map((group) => ({
      ...
      items: group.items.filter(
        (item) =>
          filterType === 'all' ||
          (filterType === 'named' && item.versionName)
      ),
    }))
  ...
  const totalVersions = filteredVersions.reduce(
  ...
  const handleRestoreVersion = async () => {
    ...
    setIsRestoring(true);
    try {
      const resp = await restoreTemplateVersionAPI(
        templateId,
        selectedVersion
      );
      if (resp && resp.success) {
        ...
        try {
          ...
          console.warn(
            'Failed to refresh versions after restore',
            e
          );
        }
        ...
        toast.error(
          resp?.message || 'Failed to restore version'
        );
      }
    } catch (e) {
      console.error('Restore failed', e);
      toast.error(
        e?.message || 'Failed to restore version'
      );
    } finally {
  ...
  const handleBookmarkVersion = (version) => {
    ...
    if (version.isBookmarked) {
      (async () => {
        try {
          const resp =
            await updateTemplateVersionBookmarkAPI(
              id,
              version.id,
              { isBookmarked: false }
            );
          if (resp?.success) {
            ...
            toast.error(
              resp?.message || 'Unbookmark failed'
            );
          }
        } catch (e) {
  ...
  const [editingNoteFor, setEditingNoteFor] =
    useState(null);
  const [editingNoteValue, setEditingNoteValue] =
    useState('');
  ...
  const saveNoteForVersion = async (versionId) => {
    ...
    try {
      ...
      if (resp?.success) {
        ...
        toast.error(
          resp?.message || 'Failed to save note'
        );
      }
    } catch (e) {
  ...
  // 🔹 Sidebar content as an inner component so we can reuse it
                  ...
                  {group.items.map((version) => (
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between ml-2">
                        ...
                        <div
                          className="relative flex items-start gap-2 flex-shrink-0 pt-1"
                          ref={
                            menuOpen === version.id
                              ? menuRef
                              : null
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-3 md:justify-end">
            ...
            {isMobile && (
              <button
                ...
                className="px-3 py-1.5 text-xs font-medium border border-gray-300 rounded-lg bg-white hover:bg-gray-50 w-full sm:w-auto"
              >
                {showHistory
        ...
        onDuplicate={async (itemWithTitle) => {
          ...
          try {
            ...
            if (itemWithTitle && itemWithTitle.id) {
              ...
              resp =
                await duplicateTemplateFromVersionAPI(
                  id,
                  itemWithTitle.id,
                  title
                );
            ...
            <div className="mb-6 space-y-3">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                <p className="text-xs text-amber-800">
                  ...
                  create a new version capturing the
                  restore. Your current version will be
