...
export default function SubmissionDetails() {
  ...
  const [removingTpl, setRemovingTpl] = useState(false);
  ...
  useEffect(() => {
    ...
    return () => { cancelled = true; };
  }, [showAddTemplate, addTplTemplates.length]);
  ...
  return (
    <>
      ...
      {showRemoveTemplateModal && templateToRemove && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-[2px] flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 border-b bg-gradient-to-r from-red-50 to-red-100 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-red-600 rounded-lg">
                  <Trash2 size={20} className="text-white" />
                </div>
                <div>
                  <h3 className="text-lg font-bold text-gray-900">Remove Template</h3>
                  <p className="text-xs text-gray-600">This only affects the bin configuration.</p>
                </div>
              </div>
              <button
                onClick={() => { setShowRemoveTemplateModal(false); setTemplateToRemove(null); }}
            ...
            <div className="px-6 py-5 space-y-3">
              <p className="text-sm text-gray-700">
                Are you sure you want to remove this template from the submission bin?
              </p>
              <div className="p-3 rounded-lg bg-gray-50 border border-gray-200 text-sm">
                <p className="font-semibold text-gray-900 truncate" title={templateToRemove.title || templateToRemove.id}>
                  {templateToRemove.title || templateToRemove.id}
                </p>
                <div className="mt-1 flex flex-wrap gap-2">
                  {templateToRemove.docCode && (
                    <span className="text-xs px-2 py-0.5 rounded bg-purple-100 text-purple-700 font-medium">
                      {templateToRemove.docCode}
                    </span>
                  )}
                  {templateToRemove.revision !== undefined && templateToRemove.revision !== null && templateToRemove.revision !== '' && (
                    <span className="text-xs px-2 py-0.5 rounded bg-green-100 text-green-700 font-medium">
                      Rev. {String(templateToRemove.revision).padStart(2, "0")}
                    </span>
                  )}
                </div>
              </div>
              <p className="text-xs text-gray-500">
                Existing submissions that already used this template will remain unchanged.
              </p>
            </div>
            <div className="px-6 py-4 border-t bg-gray-50 flex justify-end gap-3">
              <button
              ...
              <button
                type="button"
                disabled={removingTpl}
                onClick={async () => {
                  if (!templateToRemove) return;
                  try {
                    setRemovingTpl(true);
                    const removeId = String(templateToRemove.id);
                    const nextIds = (bin.template_ids || []).map(String).filter(x => x !== removeId);
                    const updated = await updateSubmissionBinAPI(bin._id || bin.id, { template_ids: nextIds });
                    setBin(updated);
                    setTemplatesInfo(prev => prev.filter(x => String(x._id || x.id) !== removeId));
                    toast.success("Template removed from bin");
