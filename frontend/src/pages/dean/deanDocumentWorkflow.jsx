    ...
    if (query.trim()) {
      ...
      out = out.filter((r) =>
        (r.id +
          r.code +
          r.title +
          (tab === "submitted" ? r.createdBy : r.ownedBy || "")
        )
          .toLowerCase()
          .includes(q)
      );
          ...
          <main className="p-5 flex-1 overflow-y-auto">
            ...
            {viewMode === "table" ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5 mt-2">
                {pageRows.map((row, i) => {
                  // Map placeholder row to the shape DocumentCard expects via "document" prop
                  const docForCard = {
                    _id: row.id || i,
                    id: row.id || i,
                    title: row.title,
                  ...
                  return (
                    <DocumentCard
                      ...
                      user={user}
                      onSelect={() =>
                        navigate(`/dean/documents/${row.id}`, {
                          state: { from: "workflow" },
                        })
                      }
                      // no-ops if card shows rename/delete
                      onRename={() => {}}
                      onDelete={() => {}}
                    />
                  );
