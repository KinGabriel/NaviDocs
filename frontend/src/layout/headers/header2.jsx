  ...
  return (
    <>
      <div className="sticky top-0 z-50 bg-[#f3f3f3] shadow-sm">
        {/* blue strip */}
        <div className="h-3 sm:h-4 bg-[#063c8d] w-full" />

        {/* main header bar */}
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between bg-[#f3f3f3] px-3 sm:px-4 lg:px-8 py-2 sm:py-3 border-b border-gray-200">
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <img
              ...
              alt="Logo"
              title="Navidocs home"
              className="w-10 h-8 sm:w-15 sm:h-10 cursor-pointer"
              onClick={() => {
            ...
            <button
              ...
              className="flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 rounded-lg text-gray-700 hover:bg-gray-200 transition-colors"
              aria-label="Back"
              title="Back"
            >
              <svg
            ...
            {/* Divider */}
            <span aria-hidden="true" className="h-5 w-px bg-gray-300 mx-0.5 hidden xs:inline-block sm:inline-block" />

            <div className="flex items-center min-w-0">
              <input
                className={`bg-transparent w-full max-w-[160px] sm:max-w-xs md:max-w-md text-base sm:text-lg md:text-xl font-medium text-gray-800 outline-none border-none truncate ${templateStatus !== 'draft' ? 'cursor-not-allowed opacity-80' : ''
                  }`}
                value={title}
          ...
          <div className="flex items-center gap-2 sm:gap-3 mt-1 sm:mt-0 flex-wrap justify-end">
            <div className="flex flex-col items-start text-[10px] leading-tight min-w-[80px]">
              {lastSavedAt && (
            ...
            <div className="relative group">
              <button
                ...
                className={`${action.className} rounded px-3 py-1.5 text-xs sm:text-sm font-semibold flex items-center gap-2 disabled:opacity-70`}
              >
                {action.icon}
                {action.label}
              </button>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-1">
                      {(approvers.length > 0 ? approvers : draftApprovers).map(a => {
                        ...
                        return (
                          <div key={a._id} className={`flex items-center justify-between bg-gray-50 border rounded px-2 py-1 ${itemStatus === 'approved' ? 'border-green-200' :
            ...
            {/* Avatar */}
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center shadow overflow-hidden bg-white border border-gray-200">
              <img
                src={
                ...
                alt="Profile"
                className={`object-cover ${user?.profile_picture ? "w-full h-full" : "w-8 h-8 object-contain opacity-90"
                  }`}
                onError={(e) => {
