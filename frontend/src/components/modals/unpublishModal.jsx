...
export default function UnpublishModal({
  open,
  onClose,
  itemType = "item", // "template", "document"
  itemTitle = "",
  onConfirm,
  submitting = false,
  error = "",
      ...
      >
        ...
        <div className="px-6 py-6 text-center">
          <p className="text-gray-700 text-sm leading-relaxed">
            ...
            ? This action will make it no longer publicly visible.
          </p>
          {(localError || error) && (
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 bg-gray-50 rounded-b-2xl">
          ...
          <button
            onClick={onConfirm}
            disabled={submitting}
            className="px-5 py-2.5 rounded-lg font-medium bg-yellow-600 text-white hover:bg-yellow-700 active:bg-yellow-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm hover:shadow flex items-center gap-2"
          >
            {submitting ? (
              <>
